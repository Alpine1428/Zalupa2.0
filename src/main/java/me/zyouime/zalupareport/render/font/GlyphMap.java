...
class GlyphMap {
    ...
    public GlyphMap(char from, char to, Font font, Identifier id, int padding) {
        fromIncl = from; toExcl = to; this.font = font; bindToTexture = id; pixelPadding = padding;
    }
    ...
    public void generate() {
        ...
        List<Glyph> gl = new ArrayList<>(); FontRenderContext frc = new FontRenderContext(new AffineTransform(), true, false);
        while (gen <= range) {
            char cc = (char)(fromIncl + gen); Font f = getFontForGlyph(cc);
            Rectangle2D sb = f.getStringBounds(String.valueOf(cc), frc);
            ...
            curRowMaxY = Math.max(curRowMaxY, h); gl.add(new Glyph(curX, curY, w, h, cc, this));
            curX += w + pixelPadding; charNX++;
        }
        ...
        g2d.setColor(new Color(255, 255, 255, 0)); g2d.fillRect(0, 0, width, height); g2d.setColor(Color.WHITE);
    ...
    public static void registerBufferedImageTexture(Identifier i, BufferedImage bi) {
        try {
            ...
            IntBuffer buf = MemoryUtil.memIntBuffer(ptr, ow * oh);
            WritableRaster ra = bi.getRaster(); ColorModel cm = bi.getColorModel();
            int nb = ra.getNumBands(), dt = ra.getDataBuffer().getDataType();
