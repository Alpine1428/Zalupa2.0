...
public class FontRenderer implements Closeable {
    ...
    public static String stripControlCodes(String text) {
        ...
        for (int i = 0; i < chars.length; i++) { char c = chars[i]; if (c == '\u00a7') { i++; continue; } f.append(c); }
        return f.toString();
    }
    private void sizeCheck() { int gs = (int) mc.getWindow().getScaleFactor(); if (gs != this.previousGameScale) { close(); init(this.font, getSizeToScale(this.originalSize, gs)); } }
    ...
    public void init(Font font, float sizePx) {
        ...
        initialized = true; this.previousGameScale = (int) mc.getWindow().getScaleFactor(); this.scaleMul = this.previousGameScale;
        this.font = font.deriveFont(sizePx * this.scaleMul);
        if (prebakeGlyphs != null && !prebakeGlyphs.isEmpty()) prebakeGlyphsFuture = this.prebake();
    }
    ...
    private GlyphMap generateMap(char from, char to) { GlyphMap gm = new GlyphMap(from, to, this.font, randomIdentifier(), padding); maps.add(gm); return gm; }
    private Glyph locateGlyph0(char glyph) {
        for (GlyphMap map : maps) if (map.contains(glyph)) return map.getGlyph(glyph);
        int base = floorNearestMulN(glyph, charsPerPage);
        return generateMap((char) base, (char) (base + charsPerPage)).getGlyph(glyph);
    }
    ...
    public void drawString(MatrixStack stack, String s, float x, float y, float r, float g, float b, float a, boolean gradient, int offset) {
        if (prebakeGlyphsFuture != null && !prebakeGlyphsFuture.isDone()) { try { prebakeGlyphsFuture.get(); } catch (Exception e) {} }
        sizeCheck(); float r2 = r, g2 = g, b2 = b;
        stack.push(); y -= 3f; stack.translate(x, y, 0); stack.scale(1f / this.scaleMul, 1f / this.scaleMul, 1f);
        ...
        float xOffset = 0, yOffset = 0; boolean inSel = false; int lineStart = 0;
        synchronized (GLYPH_PAGE_CACHE) {
            for (int i = 0; i < chars.length; i++) {
                ...
                else if (c == '\n') { yOffset += getStringHeight(s.substring(lineStart, i)) * scaleMul; xOffset = 0; lineStart = i + 1; continue; }
                Glyph glyph = locateGlyph1(c);
                if (glyph != null) { if (glyph.value() != ' ') { GLYPH_PAGE_CACHE.computeIfAbsent(glyph.owner().bindToTexture, k -> new ObjectArrayList<>()).add(new DrawEntry(xOffset, yOffset, r2, g2, b2, glyph)); } xOffset += glyph.width(); }
            }
            for (Identifier id : GLYPH_PAGE_CACHE.keySet()) {
                RenderSystem.setShaderTexture(0, id); List<DrawEntry> objects = GLYPH_PAGE_CACHE.get(id);
                BufferBuilder bb = Tessellator.getInstance().getBuffer(); bb.begin(DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
                for (DrawEntry obj : objects) {
                    Glyph gl = obj.toDraw; GlyphMap ow = gl.owner();
                    float u1 = (float) gl.u() / ow.width, v1 = (float) gl.v() / ow.height;
                    float u2 = (float) (gl.u() + gl.width()) / ow.width, v2 = (float) (gl.v() + gl.height()) / ow.height;
    ...
    public void drawCenteredString(MatrixStack stack, String s, double x, double y, int color) { drawString(stack, s, (float) (x - getStringWidth(s) / 2f), (float) y, ((color >> 16) & 0xff) / 255f, ((color >> 8) & 0xff) / 255f, (color & 0xff) / 255f, ((color >> 24) & 0xff) / 255f); }
    ...
    @Override public void close() { try { if (prebakeGlyphsFuture != null && !prebakeGlyphsFuture.isDone() && !prebakeGlyphsFuture.isCancelled()) { prebakeGlyphsFuture.cancel(true); prebakeGlyphsFuture.get(); prebakeGlyphsFuture = null; } for (GlyphMap m : maps) m.destroy(); maps.clear(); allGlyphs.clear(); initialized = false; } catch (Exception e) { e.printStackTrace(); } }
