...
public class AutoCallManager {
    ...
    public void reset() {
        cancelled = true; state = State.IDLE; currentNick = null;
        ptStart = 0; waitFind = false; waitPt = false; foundAny = false;
        scanningPlaytime = false; closeReason = CloseReason.PLAYER_AFK;
        CommandQueue.clear();
    }
    ...
    public void startAutoCall() {
        ...
        cancelled = false; state = State.SEARCHING; foundAny = false; search();
    }
    ...
    private void search() {
        ...
        if (slot != -1 && nick != null) {
            ...
            if (hasNext) {
                ...
                if (!foundAny) { if (client.player != null) client.player.closeHandledScreen(); msg("\u00a7c[Auto] \u041d\u0435 \u043d\u0430\u0439\u0434\u0435\u043d\u043e"); }
                state = State.IDLE;
            }
        }
    }
    ...
    public void onChatMessage(String message) {
        ...
        if (state == State.DOING_FIND && waitFind) {
            ...
            if (m.find()) { waitFind = false; connect(m.group(1).trim().toLowerCase()); return; }
        }
        ...
        if (config.autoCheck && (state == State.DOING_PLAYTIME || state == State.CHECKING_PLAYTIME_LOOP) && waitPt) {
            if (message.contains("\u041f\u043e\u0441\u043b\u0435\u0434\u043d\u044f\u044f \u0430\u043a\u0442\u0438\u0432\u043d\u043e\u0441\u0442\u044c")) {
                ...
                if (m.find()) {
                    ...
                    handlePlaytimeResult(h * 3600 + min * 60 + sec); return;
                }
            }
        }
        ...
        if (config.autoCheck && state == State.WAITING_SPYFRZ) {
            if (openReportPattern.matcher(message).find()) {
                ...
                state = State.WAITING_REPORT_OPEN; return;
            }
        }
        ...
        if (config.autoCheck && (state == State.WAITING_REPORT_OPEN || state == State.WAITING_SPYFRZ)) {
            if (afkServerPattern.matcher(message).find()) {
                ...
                state = State.CLOSING_STEP1; delay(this::closeStep1, 1000); return;
            }
    ...
    public void onOutgoingCommand(String command) {
        ...
        if (config.autoCheck && (state == State.WAITING_SPYFRZ || state == State.WAITING_REPORT_OPEN)) {
            if (banOutgoing.matcher(command).find()) {
                msg("\u00a7a[Auto] \u0411\u0430\u043d. \u0417\u0430\u0432\u0435\u0440\u0448\u0430\u044e...");
                closeReason = CloseReason.PLAYER_BANNED;
                state = State.CLOSING_STEP1; delay(this::closeStep1, 2000); return;
            }
    ...
    private void doSpy() {
        ...
        state = State.DOING_FIND; delay(this::doFind, 3000);
    }
    ...
    private void doFind() {
        ...
        delay(() -> {
            if (waitFind && state == State.DOING_FIND) {
                waitFind = false; msg("\u00a7c[Auto] \u0422\u0430\u0439\u043c\u0430\u0443\u0442 /find");
                if (config.autoCheck) { closeReason = CloseReason.PLAYER_AFK; state = State.CLOSING_STEP1; delay(AutoCallManager.this::closeStep1, 1000); }
    ...
    private void connect(String srv) {
        ...
        if (command == null) {
            ...
            if (config.autoCheck) { closeReason = CloseReason.PLAYER_AFK; state = State.CLOSING_STEP1; delay(this::closeStep1, 1000); }
            else state = State.IDLE;
            return;
        }
        msg("\u00a7e[Auto] /" + command); CommandQueue.add(command);
        if (config.autoCall && !config.autoCheck) { msg("\u00a7a[Auto] \u0417\u0430\u0432\u0435\u0440\u0448\u0435\u043d."); state = State.IDLE; return; }
        if (config.autoCheck) { state = State.CONNECTING_SERVER; msg("\u00a7e[Auto] 10\u0441..."); delay(this::startPlaytimeCheck, 10000); }
    }

    private void startPlaytimeCheck() {
        if (cancelled) { state = State.IDLE; return; }
        state = State.DOING_PLAYTIME; ptStart = System.currentTimeMillis();
        scanningPlaytime = true; msg("\u00a7e[Auto] \u041f\u043b\u0435\u0439\u0442\u0430\u0439\u043c..."); sendPlaytime();
    }

    private void sendPlaytime() {
        if (cancelled || currentNick == null) { scanningPlaytime = false; state = State.IDLE; return; }
        waitPt = true; CommandQueue.add("playtime " + currentNick);
    }
    ...
    private void handlePlaytimeResult(int lastActivitySec) {
        ...
        if (lastActivitySec < 7) {
            ...
            CommandQueue.add("hm spyfrz"); state = State.WAITING_SPYFRZ;
        } else {
            if (state == State.DOING_PLAYTIME) {
                state = State.CHECKING_PLAYTIME_LOOP; ptStart = System.currentTimeMillis();
                msg("\u00a7e[Auto] \u0410\u0424\u041a (" + lastActivitySec + "\u0441). 30\u0441...");
            }
            long elapsed = System.currentTimeMillis() - ptStart;
            if (elapsed < 30000) {
                delay(() -> { if (state == State.CHECKING_PLAYTIME_LOOP && !cancelled) sendPlaytime(); }, 3000);
            } else {
                scanningPlaytime = false; msg("\u00a7c[Auto] \u0410\u0424\u041a >30\u0441.");
                closeReason = CloseReason.PLAYER_AFK; state = State.CLOSING_STEP1; delay(this::closeStep1, 1000);
            }
        }
    }
    ...
    private void closeStep1() {
        if (cancelled) { state = State.IDLE; return; }
        scanningPlaytime = false; msg("\u00a7e[Auto] /reportlist");
        CommandQueue.add("reportlist"); state = State.CLOSING_STEP2; delay(this::closeStep2, 2000);
    }

    private void closeStep2() {
        if (cancelled) { state = State.IDLE; return; }
        if (client.player == null || client.player.currentScreenHandler == null) { msg("\u00a7c[Auto] err step2"); state = State.IDLE; return; }
        msg("\u00a7e[Auto] LKM slot 47");
    ...
    private void closeStep3() {
        ...
        msg("\u00a7e[Auto] LKM slot " + slotName + " (" + closeReason.name() + ")");
        ...
        state = State.CLOSING_STEP4; delay(this::closeStep4, 1000);
    }
    ...
    private void reopen() {
        ...
        foundAny = false; currentNick = null; closeReason = CloseReason.PLAYER_AFK;
        msg("\u00a7a[Auto] \u0421\u043b\u0435\u0434\u0443\u044e\u0449\u0438\u0439...");
        CommandQueue.add("reportlist");
        delay(() -> { if (cancelled) { state = State.IDLE; return; } state = State.SEARCHING; search(); }, 1500);
    }

    private void msg(String m) { if (client.player != null) client.execute(() -> client.player.sendMessage(Text.of(m))); }
    private void delay(Runnable r, long ms) {
        Thread t = new Thread(() -> { try { Thread.sleep(ms); } catch (InterruptedException e) { return; } if (!cancelled) client.execute(r); });
