...
public class AutoCallManager {
    ...
    public enum State {
        ...
        DOING_FIND,
        CONNECTING_SERVER,
        DOING_PLAYTIME,          // первая проверка плейтайма
        CHECKING_PLAYTIME_LOOP,  // цикл проверки 30 секунд
        WAITING_SPYFRZ,
        WAITING_REPORT_OPEN,
        CLOSING_STEP1,
        CLOSING_STEP2,
    ...
    private long playtimeLoopStartTime;  // время начала 30-секундного цикла проверки
    ...
    // Счётчик проверок плейтайма для лога
    ...
    public void onChatMessage(String message) {
        ...
        // Ответ на /playtime — обработка результата активности
        ...
        // "Открываем репорт" -> переход в WAITING_REPORT_OPEN
        ...
        // Отслеживание событий во время проверки (бан/анфриз/афк сервера)
    ...
    private void connect(String srv) {
        ...
        // Если только autoCall (без autoCheck) — завершаем
        ...
        // Если autoCheck — ждём 10 секунд и начинаем проверку плейтайма
        if (config.autoCheck) {
    ...
    /**
     * Начинаем 30-секундный цикл проверки плейтайма.
     * Таймер начинается ЗДЕСЬ — с первой проверки.
     * Каждые 3 секунды отправляем /playtime.
     * Если хотя бы раз активность < 7 сек -> игрок активен -> spyfrz.
     * Если за 30 секунд ни разу не стал активен -> АФК -> закрываем репорт.
     */
    ...
    /**
     * Обработка результата проверки плейтайма.
     * 
     * ЛОГИКА:
     * - Если lastActivitySec < 7  -> ИГРОК АКТИВЕН -> сразу /hm spyfrz (вызываем на проверку)
     * - Если lastActivitySec >= 7 -> ИГРОК АФК в этот момент
     *     -> Проверяем: прошло ли 30 секунд с начала цикла?
     *        -> НЕТ -> ждём 3 секунды и проверяем снова
     *        -> ДА  -> за все 30 секунд игрок был АФК -> закрываем репорт по причине "afk"
     */
    private void handlePlaytimeResult(int lastActivitySec) {
        ...
        // Проверяем: убедимся что мы в правильном состоянии
        if (state != State.CHECKING_PLAYTIME_LOOP && state != State.DOING_PLAYTIME) {
            return;
        }

        // Если были в DOING_PLAYTIME — переключаем в LOOP
        ...
        if (lastActivitySec < 7) {
            // ===== ИГРОК АКТИВЕН =====
            scanningPlaytime = false;
            msg("\u00a7a[Auto] \u0418\u0433\u0440\u043e\u043a \u0430\u043a\u0442\u0438\u0432\u0435\u043d! (" + lastActivitySec + "\u0441, \u043f\u0440\u043e\u0432\u0435\u0440\u043a\u0430 #" + playtimeCheckCount + ", " + (elapsed / 1000) + "\u0441 \u043f\u0440\u043e\u0448\u043b\u043e)");
            msg("\u00a7a[Auto] \u0412\u044b\u0437\u044b\u0432\u0430\u044e \u043d\u0430 \u043f\u0440\u043e\u0432\u0435\u0440\u043a\u0443: /hm spyfrz");
            CommandQueue.add("hm spyfrz");
            state = State.WAITING_SPYFRZ;
        } else {
            // ===== ИГРОК АФК В ЭТОТ МОМЕНТ =====
            msg("\u00a7e[Auto] \u0410\u0424\u041a (" + lastActivitySec + "\u0441, \u043f\u0440\u043e\u0432\u0435\u0440\u043a\u0430 #" + playtimeCheckCount + ", " + (elapsed / 1000) + "\u0441/" + "30\u0441)");

            if (elapsed < 30000) {
                // Ещё не прошло 30 секунд — ждём 3 сек и проверяем снова
                delay(() -> {
                ...
                // ===== 30 СЕКУНД ПРОШЛО, ИГРОК ВСЁ ВРЕМЯ БЫЛ АФК =====
                scanningPlaytime = false;
                msg("\u00a7c[Auto] \u0418\u0433\u0440\u043e\u043a \u0431\u044b\u043b \u0410\u0424\u041a \u0432\u0441\u0435 30 \u0441\u0435\u043a\u0443\u043d\u0434 (" + playtimeCheckCount + " \u043f\u0440\u043e\u0432\u0435\u0440\u043e\u043a). \u0417\u0430\u043a\u0440\u044b\u0432\u0430\u044e \u0440\u0435\u043f\u043e\u0440\u0442...");
                closeReason = CloseReason.PLAYER_AFK;
                state = State.CLOSING_STEP1;
                delay(this::closeStep1, 1000);
            }
    ...
    // ==================== ЗАВЕРШЕНИЕ (ЗАКРЫТИЕ РЕПОРТА) ====================

    /**
     * Шаг 1: Открываем /reportlist
     */
    private void closeStep1() {
        if (cancelled) { state = State.IDLE; return; }
        scanningPlaytime = false;
        msg("\u00a7e[Auto] \u041e\u0442\u043a\u0440\u044b\u0432\u0430\u044e /reportlist...");
    ...
    /**
     * Шаг 2: Клик на слот 47 (индекс 46) — "завершить репорт"
     */
    private void closeStep2() {
        if (cancelled) { state = State.IDLE; return; }
        if (client.player == null || client.player.currentScreenHandler == null) {
            msg("\u00a7c[Auto] \u041e\u0448\u0438\u0431\u043a\u0430 step2: \u043c\u0435\u043d\u044e \u043d\u0435 \u043e\u0442\u043a\u0440\u044b\u0442\u043e");
            state = State.IDLE;
            return;
        }
    ...
    /**
     * Шаг 3: Клик на причину закрытия
    ...
    private void closeStep3() {
        if (cancelled) { state = State.IDLE; return; }
        if (client.player == null || client.player.currentScreenHandler == null) {
            msg("\u00a7c[Auto] \u041e\u0448\u0438\u0431\u043a\u0430 step3: \u043c\u0435\u043d\u044e \u043d\u0435 \u043e\u0442\u043a\u0440\u044b\u0442\u043e");
            state = State.IDLE;
            return;
        }
        int slotIndex;
        ...
        msg("\u00a7e[Auto] \u041a\u043b\u0438\u043a \u0441\u043b\u043e\u0442 " + slotName + " (\u043f\u0440\u0438\u0447\u0438\u043d\u0430: " + closeReason.name() + ")");
    ...
    /**
     * Шаг 4: Закрываем GUI и отправляем сообщение в чат
     * AFK / AFK_SERVER -> "afk"
     * BANNED / UNFROZEN -> "-"
     */
    private void closeStep4() {
        ...
        // Ждём закрытие GUI, потом отправляем сообщение
        delay(() -> {
            if (cancelled) { state = State.IDLE; return; }
            ...
            // Снимаем spy и идём к следующему репорту
            delay(() -> {
                if (cancelled) { state = State.IDLE; return; }
