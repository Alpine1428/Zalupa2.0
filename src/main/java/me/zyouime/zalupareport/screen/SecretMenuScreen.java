...
public class SecretMenuScreen extends Screen {
    ...
    public SecretMenuScreen() { super(Text.literal("")); config = ZalupareportClient.getInstance().config; }

    @Override protected void init() { bw=160; bh=24; bx=(width-bw)/2f; by=(height-bh)/2f; }

    @Override public void render(DrawContext ctx, int mx, int my, float d) {
        renderBackground(ctx); MatrixStack ms = ctx.getMatrices();
    ...
    @Override public boolean mouseClicked(double mx, double my, int b) {
        if(b==0&&mx>=bx&&mx<=bx+bw&&my>=by&&my<=by+bh){config.autoCall=!config.autoCall;me.shedaniel.autoconfig.AutoConfig.getConfigHolder(ModConfig.class).save();return true;}
        return super.mouseClicked(mx,my,b);
    }

    @Override public boolean shouldPause(){return false;}
    @Override public boolean keyPressed(int k,int s,int m){if(k==org.lwjgl.glfw.GLFW.GLFW_KEY_ESCAPE||k==org.lwjgl.glfw.GLFW.GLFW_KEY_RIGHT_CONTROL){close();return true;}return super.keyPressed(k,s,m);}
}
